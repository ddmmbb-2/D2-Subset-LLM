...
print(f"📚 詞表大小: {vocab_size} | 總訓練字數: {len(data)}")
...
# ==========================================
print("🚀 V11 + QLLM2 轉生計畫啟動！(啟動 RTX 3060 專屬 BF16 絕對防護)")
model.train()

global_step = 0

# 🌟 使用 tqdm 包裝訓練迴圈
pbar = tqdm(range(global_step, config["epochs"]), desc="🧠 V11 訓練中", dynamic_ncols=True)
...
for step in pbar:
    ...
    if global_step % 2000 == 0:
        ...
        tqdm.write(f"💾 Step {global_step} 模型已自動儲存至 {config['save_model']}")
